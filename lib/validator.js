...
//Validate over transfer data function and also set session for recipientBudget and senderBudget
  ...
  async function validateTransferIndividual(req, res, next){
  	...
  	const recipientEmail = req.body.recipient.trim();
  	const senderEmail = req.user.email.trim();
  	const transferReason = req.body.reason.trim();

    //
    if(recipientEmail == senderEmail) {
      return res.status(400).send("Can't send to yourself!")
    }
  ...
  async function validateSectionRefund(req, res, next){
  	//intialize data
  	const transferBudget = req.body.amount;
  	const recipientEmail = req.body.recipient.trim();
  	const senderEmail = req.body.sender.trim();
  	const transferReason = req.body.reason.trim();
  ...
  async function validateTransferApartment(req, res, next){
    ...
    if (apartmentEmail == recipientEmail) {
        return res.status(400).send("You can't send to your own apartment!")
    }
...
module.exports = {
  ...
  sectionRefund: validateSectionRefund
}
