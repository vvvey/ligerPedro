...
module.exports.set = function(router, pool)  {
	router.get('/catering', (req, res) => {
		res.redirect('/catering/overview');
	});
	...
	router.get('/catering/transfer_logs', ensureLoggedIn, User.isRole('admin', 'maintenance_manager', 'catering_manager'), (req, res) => {
	    ...
	    var paginateArray = []
	    pool.query("SELECT count(id) from transfer_logs WHERE recipient = 'catering@ligercambodia.org' AND finished = 'true' AND canceled = 'false';", (err, result) => {
	...
	router.get('/catering/overview', ensureLoggedIn, User.isRole('admin', 'maintenance_manager', 'catering_manager'), (req, res) => {
		...
		var recentTransfer  = {
			text: "	SELECT transfer_logs.*, account.username as sender_username FROM transfer_logs \
					...
					WHERE recipient = 'catering@ligercambodia.org' AND finished = 'true' AND canceled = 'false'  \
		...
		var select = {
			text: "	 SELECT account.apartment, SUM(transfer_logs.amount) \
					...
					WHERE transfer_logs.recipient = 'catering@ligercambodia.org' AND transfer_logs.finished = 'true' AND transfer_logs.canceled = 'false' \
